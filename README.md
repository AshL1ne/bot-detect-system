...
- **用户列表**：关键词搜索（需点击搜索）、**是否认证 / 是否恶意** 筛选（变更即刷新）；**管理员**在「是否恶意」列切换开关后，点**该行「保存」**再提交。详情含词云、按时段活跃图、推文分页。
- **关系**：粉丝 / 关注列表接口与页面；顶部导航 **← 返回**；用户列表页 `keep-alive` 保留分页与筛选状态。
- **管理后台**：系统登录账号（角色、启用状态；不可禁用本人；**可删除普通用户** `USER`，需确认，且不能删自己或其它管理员）。微博用户恶意标签在主页用户列表由管理员逐行保存。
